...
const steps = [
  {
    number: "01 - STEP",
    title: "Diagnose.",
    body: "We map the system before we touch it. Protocol dependencies, data flows, constraints, bottlenecks, and failure points are identified and documented. Nothing is assumed.",
  },
  {
    number: "02 - STEP",
    title: "Build.",
    body: "We implement against defined performance, reliability, and security targets. Every chain-facing and backend layer is intentional. Scope does not drift.",
  },
  {
    number: "03 - STEP",
    title: "Scale.",
    body: "We harden until the system is boring. Boring means stable. Stable means done. Growth, chain activity, and load are no longer events - they are conditions the system was built for.",
  },
];

const principlesList = [
  { text: "Performance over ", accent: "abstraction.", accentColor: "#02FFE4" },
  { text: "Systems must survive chain activity, not ", accent: "demos.", accentColor: "#008F82" },
        ...
        >
          {steps.map((step, i) => (
            <motion.div
              ...
              style={{
                borderRight: i < steps.length - 1 ? "0.5px solid #BFD8D3" : "none",
...
export function Principles() {
  return (
    <section id="principles" className="bg-background py-24 md:py-32 px-4 md:px-6 w-full">
      <div className="max-w-7xl mx-auto">
        <span className="text-primary text-sm uppercase tracking-widest font-medium block mb-12">
          Principles
        </span>
        ...
        <div style={{ borderTop: "0.5px solid #BFD8D3" }}>
          {principlesList.map((item, i) => (
            <motion.div
              ...
              style={{ borderBottom: "0.5px solid #BFD8D3" }}
            >
              <p
              ...
              <span
                className="font-mono text-sm ml-8 flex-shrink-0 tabular-nums"
